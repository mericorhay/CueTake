...
- Native Apple authorization → verified Worker exchange → Keychain session → account management.
...
| `POST /auth/apple` | Apple RS256 signature/JWKS, issuer, audience, nonce, issue/expiry times; single-use challenge; server authorization-code exchange and subject match |
...
| `DELETE /auth/account` | Revokes Apple refresh token, deletes account and **all** sessions |

Sessions are random 256-bit values, stored only as SHA-256 hashes in D1, and expire after 30 days
(reauthentication, not indefinite silent renewal). Apple's refresh credential is AES-256-GCM
encrypted, bound to the account ID as authenticated data. No email is requested or stored. The
...
2. Create a Sign in with Apple key associated with this primary App ID. Keep its `.p8` private;
   this is **not** the App Store Connect upload key. Team ID: `XYB3NLV654`.
3. Set Worker secrets from the secure local environment:
   - `APPLE_AUTH_KEY_ID`
...
4. Regenerate the App Store provisioning profile with Sign in with Apple enabled; replace GitHub
   `IOS_PROVISIONING_PROFILE` with that profile's base64, preserving the other capabilities.
5. Run CI, then TestFlight. The release workflow inspects the **actual profile**, derives temporary
...
6. Verify on a real iPhone: fresh authorization, cancel, returning authorization (no name supplied),
...
`INFOPLIST_KEY_CueTakeAppleSignInEnabled = YES`. Do not turn on this flag without the entitlement.
